...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="email"], select { width: 300px; padding: 8px; border: 1px solid #ccc; border-radius: 4px; }
        .delete-btn { background-color: #dc3545; color: white; padding: 10px 15px; border: none; cursor: pointer; border-radius: 4px; }
    </style>
</head>
<body>

    <h1>Gestion de l'Utilisateur : <?= htmlspecialchars($user['login']) ?> (<?= htmlspecialchars($user['status']) ?>)</h1>
    ...
    <form method="POST">
        ...
        <div>
            ...
            <select id="role" name="role" required>
                ...
                <option value="pending" <?= ($user['role'] == 'pending') ? 'selected' : '' ?>>En attente</option>
            </select>
        </div>
